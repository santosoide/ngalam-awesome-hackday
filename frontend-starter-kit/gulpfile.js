...
var settings = {
	...
	watch: ['./src/**/*.html'/*, './framework/controllers/*.php', …*/],
...
gulp.task('default', function (callback) {
	global.watch = true;
	global.open = true;
	gulpSequence(['sass', 'move', 'move-html', settings.jsTasker], ['watcher', 'browserSync'], callback);
});
...
gulp.task('build', function (callback) {
	...
	gulpSequence(['sass', 'move', 'move-html', settings.jsTasker], callback);
});
...
gulp.task('move', ['move-html'], function () {
...
gulp.task('watcher', ['browserSync'], function () {
	watch(settings.watch, function () { gulp.start('move-html'); });
